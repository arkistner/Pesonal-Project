...
using UnityEngine;
public class _44_MagnumGun : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //shooting unscoped
        if (Input.GetButtonDown("Fire1") && isScoped == false)
        {
            StartCoroutine(StartRecoil());
        }
        //shooting scoped
        if (Input.GetButtonDown("Fire1") && isScoped == true)
        {
            StartCoroutine(StartScopeRecoil());
        }
        // unscoping while shooting
        if (Input.GetButtonDown("Fire2") && isShootingScoped == true && isScoped == true)
        {
            StartCoroutine(ScopeWhileShooting());
        }
        //scoping while shooting
        if (Input.GetButtonDown("Fire2") && isShootingUnscoped == true && isScoped == false)
        {
            StartCoroutine(UnscopeWhileShooting());
    ...
    IEnumerator ScopeWhileShooting()
    {
        yield return new WaitForSeconds(.0f);
        isShootingScoped = true;
        isShootingUnscoped = false;
    ...
    IEnumerator UnscopeWhileShooting()
    {
        yield return new WaitForSeconds(.0f);
        isShootingScoped = false;
        isShootingUnscoped = true;
